...
def plot_queue_and_agg_time(input_directory, output_directory):
    try:
        ...
        # Iterate over files in the queue directory
        ...
        for file in files:
            if file.startswith("con0_queue"):
                ...
                #data['Queue'] = data['Queue'] / 1_000  # Convert queue to milliseconds
                queue_data[flow] = data
        ...
        for key, df in queue_data.items():
            ax1.plot(df['Time'], df['Queue'], label=f'Queue - {key}')
        ...
        ax1.set_title('Consumer: Queue vs. Aggregation Time', fontsize=16)
        ...
        output_file_path = os.path.join(output_directory, "con_queue_aggTime.png")
