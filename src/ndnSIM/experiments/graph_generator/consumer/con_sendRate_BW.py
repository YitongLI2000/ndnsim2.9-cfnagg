...
def plot_sendrate_and_bw(input_directory, output_directory):
    try:
        ...
        for flow, data in flow_data.items():
            ...
            ax2.plot(data['Time'], data['BW'], label=f'BW {flow}', color='green', linestyle='--')

            ax1.set_xlabel('Time (seconds)', fontsize=14)
            ax1.set_ylabel('SendRate (packets/ms)', fontsize=14, color='blue')
            ax2.set_ylabel('BW (packets/ms)', fontsize=14, color='green')
            plt.title(f"SendRate vs BW - Consumer's Flow {flow}", fontsize=16)
            ...
            # Save the plot
            output_file = os.path.join(output_directory, f"con_sendrate_bw_{flow}.png")
