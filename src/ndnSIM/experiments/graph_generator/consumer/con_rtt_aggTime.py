...
def plot_rtt_and_agg_time(input_directory, output_directory):
    try:
        ...
        # Iterate over files in the RTT directory
        files = os.listdir(input_directory)
        for file in files:
            if file.startswith("con0_RTT"):
                parts = file.split('_')
                ...
                #data['RTT'] = data['RTT'] / 1_000  # Convert RTT to milliseconds
                rtt_data[flow] = data
        ...
        for key, df in rtt_data.items():
            ax1.plot(df['Time'], df['RTT'], label=f'RTT - {key}')
        ...
        ax1.set_title('Consumer: RTT vs. Aggregation Time', fontsize=16)
        ...
        output_file_path = os.path.join(output_directory, "con_rtt_aggTime.png")
