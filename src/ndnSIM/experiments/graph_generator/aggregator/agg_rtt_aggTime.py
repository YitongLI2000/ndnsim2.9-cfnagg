...
def plot_rtt_and_agg_time(input_directory, output_directory):
    try:
        ...
        for file in files:
            if file.endswith(".txt") and "RTT" in file:
                parts = file.split('_')
                ...
                # Read RTT data from the file
                data = pd.read_csv(file_path, sep="\s+", header=None, names=["Time", "RTT"], usecols=[0, 2])
                data['Time'] = data['Time'] / 1_000  # Convert time to seconds for uniformity
                #data['RTT'] = data['RTT'] / 1_000  # Convert RTT to seconds

                # Use a tuple (aggregator, flow) as the key to handle multiple aggregators and flows
                rtt_data[(aggregator, flow)] = data
        ...
        for file in files:
            if file.endswith("aggregationTime.txt"):
                ...
                ax1 = plt.gca()  # Get current axis for RTT
                ax2 = ax1.twinx()  # Create another y-axis for the aggregation time

                # Plot RTT data for each flow on the left y-axis
                for key, df in rtt_data.items():
                    if key[0] == aggregator:  # Check if the data belongs to the current aggregator
                        # Add line plot for RTT
                        ax1.plot(df['Time'], df['RTT'], label=f'RTT - {key[1]}', alpha=0.7)
                        # Add scatter points to emphasize RTT data points
                        #ax1.scatter(df['Time'], df['RTT'], s=10, label=f'RTT Points - {key[1]}', alpha=0.6)
                ...
                ax1.set_ylabel('RTT (ms)', fontsize=14)
                ax2.set_ylabel('Aggregation Time (ms)', fontsize=14)
                ax1.set_title(f'RTT vs. Aggregation Time for {aggregator}', fontsize=16)
                ...
                output_file_path = os.path.join(output_directory, f"{aggregator}_rtt_aggTime.png")
