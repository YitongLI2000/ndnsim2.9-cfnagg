...
def plot_inFlight(input_directory, output_directory):
    try:
        file_prefix = "agg"
        aggregator_data = {}
        ...
        for file in files:
            if file.startswith(file_prefix):
                ...
                if 'queue' in parts:
                    ...
                    #data['inFlight'] = data['inFlight'] / 1_000  # Convert to seconds
        ...
        for aggregator_index, data_dict in aggregator_data.items():
            ...
            for key, df in data_dict.items():
                plt.plot(df['Time'], df['InFlight'], label=f'Flow {key}')

            plt.xlabel('Time (seconds)', fontsize=14)
            plt.ylabel('inFlight (packets)', fontsize=14)
            plt.title(f'inFlight - Aggregator {aggregator_index}', fontsize=16)
            ...
            # Save the plot
            output_file = os.path.join(output_directory, f"agg{aggregator_index}_inflight.png")
