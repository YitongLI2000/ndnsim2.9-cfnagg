    ...
    try:
        ...
        for aggregator_index, data_dict in aggregator_data.items():
            ...
            for key, df in data_dict.items():
                plt.plot(df['Time'], df['Throughput'], label=f'Flow {key}')

            plt.xlabel('Time (seconds)', fontsize=14)
            plt.ylabel('Throughput (Mbps)', fontsize=14)
            plt.title(f'Throughput - Aggregator {aggregator_index}', fontsize=16)
            ...
            # Save the plot
            output_file = os.path.join(output_directory, f"agg{aggregator_index}_throughput.png")
