...
def plot_rto(input_directory, output_directory):
    try:
        file_prefix = "agg"
        aggregator_data = {}
        ...
        for file in files:
            if file.startswith(file_prefix):
                ...
                if 'RTO' in parts:
                    ...
                    data = pd.read_csv(file_path, sep="\s+", header=None, names=["Time", "RTO"], usecols=[0, 1])
                    #data['RTO'] = data['RTO'] / 1_000  # Convert to seconds
        ...
        for aggregator_index, data_dict in aggregator_data.items():
            ...
            for key, df in data_dict.items():
                plt.plot(df['Time'], df['RTO'], label=f'Flow {key}')

            plt.xlabel('Time (seconds)', fontsize=14)
            plt.ylabel('RTO (ms)', fontsize=14)
            plt.title(f'RTO - Aggregator {aggregator_index}', fontsize=16)
            ...
            # Save the plot
            output_file = os.path.join(output_directory, f"agg{aggregator_index}_rto.png")
