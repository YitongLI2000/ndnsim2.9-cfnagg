...
def plot_rtt(input_directory, output_directory):
    try:
        file_prefix = "agg"
        aggregator_data = {}
        ...
        for file in files:
            if file.startswith(file_prefix):
                ...
                if 'RTT' in parts:
                    ...
                    #data['RTT'] = data['RTT'] / 1_000  # Convert to seconds
        ...
        for aggregator_index, data_dict in aggregator_data.items():
            ...
            for key, df in data_dict.items():
                plt.plot(df['Time'], df['RTT'], label=f'Flow {key}')

            plt.xlabel('Time (seconds)', fontsize=14)
            plt.ylabel('RTT (ms)', fontsize=14)
            plt.title(f'RTT - Aggregator {aggregator_index}', fontsize=16)
            ...
            # Save the plot
            output_file = os.path.join(output_directory, f"agg{aggregator_index}_rtt.png")
